...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
		...
		// --- Attach Image Button ---
		const attachImageButton = dom.$('button.chat-attach-image-button', { title: 'Attach image', type: 'button', style: 'margin-right: 4px; display: flex; align-items: center;' });
		attachImageButton.innerHTML = `<span class="codicon codicon-file-media"></span>`;
		const fileInput = dom.$('input', { type: 'file', accept: 'image/*', style: 'display: none;' }) as HTMLInputElement;
		attachImageButton.onclick = () => fileInput.click();
		fileInput.onchange = async (e) => {
			...
			if (file) {
				...
				const displayName = file.name || 'Uploaded Image';
				// Deduplicate by hash
				const id = await imageToHash(uint8Array);
				if (!this.attachmentModel.attachments.find(a => a.id === id)) {
		...
		toolbarsContainer.prepend(attachImageButton);
		toolbarsContainer.appendChild(fileInput);
	...
	async renderChatEditingSessionState(chatEditingSession: IChatEditingSession | null, chatWidget?: IChatWidget) {
		...
		// --- Working Set Pills (Changed/Modified Files Only) ---
		// Filter to only show files that are actually changed/modified and have a 'reference' property
		const changedFiles = entries.filter(e => e.kind === 'reference' && e.state !== WorkingSetEntryState.Suggested && !e.excluded && 'reference' in e && e.reference);
		let collapsed = true;
		const maxVisible = 3;
		const pillsContainer = document.createElement('div');
		pillsContainer.className = 'working-set-pills-container';
		const renderPills = () => {
			...
			filesToShow.forEach((entry, idx) => {
				...
				rejectIcon.className = 'working-set-pill-reject codicon codicon-discard';
				rejectIcon.title = 'Revert changes';
			...
			if (changedFiles.length > maxVisible) {
				...
				togglePill.className = 'working-set-pill working-set-toggle';
				togglePill.textContent = collapsed ? `+${changedFiles.length - maxVisible} more` : 'Show less';
				togglePill.onclick = () => {
					collapsed = !collapsed;
					renderPills();
				};
				pillsContainer.appendChild(togglePill);
			}
		};
		renderPills();
		// Clear previous and append pills
		const pillsWorkingSetContainer = innerContainer.querySelector('.chat-editing-session-list') as HTMLElement ?? dom.append(innerContainer, $('.chat-editing-session-list'));
		pillsWorkingSetContainer.innerHTML = '';
