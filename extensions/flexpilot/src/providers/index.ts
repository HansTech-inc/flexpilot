...
// Instantiate AgentTools once for all providers
		...
		const disposable = vscode.lm.registerChatModelProvider(
			configId,
			{
				...
				async provideLanguageModelResponse(
					...
					// Merge agent tools into the tools array for tool-calling models
					const allTools = [
						...(options.tools || []),
						...Array.from(agentTools.getTools().values()).map(tool => ({
							name: tool.name,
